...
const SCREENS = [
  ...
  },
  {
    ...
    /* PoW 체험 직후 복귀 화면. 방금 한 조작을 한 줄로 회수한다.
       반복 루프 도식은 다음 화면(L16)이 맡는다. 두 화면이 같은 그림을 두 번 그리지 않는다. */
    ...
    body: [
      ...
      /* 이 화면은 답하지 않고 질문으로 끝낸다. 답은 다음 화면이다 */
      bigQuestion('누가 먼저 찾을 가능성이 높을까?')
    ].join('')
  },
  {
    ...
    body: [
      /* 앞 화면이 던진 질문을 그대로 받아 답한다 */
      recall('앞 화면의 질문', 'A 초당 10회 &nbsp;·&nbsp; B 초당 1,000,000회 — 누가 먼저 찾을 가능성이 높을까?'),
      loopFlow({
        steps: ['숫자 변경', 'Hash 계산'],
