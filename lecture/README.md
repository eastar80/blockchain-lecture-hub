...
`materials/lecture.pdf` 는 이 34화면을 그대로 담은 PDF 입니다.
수강생 배포용이자 강사 비상 백업이며, 사이트에서 여는 유일한 PDF 입니다.

실제 Stage 를 `?preview=1` 로 띄워 한 문서로 인쇄한 것이라 **글자를 선택·검색할 수 있습니다.**
화면을 다시 그리지 않으므로 PDF 와 프로젝터 화면이 어긋나지 않습니다.
화면을 고치면 PDF 도 다시 뽑아야 합니다.
